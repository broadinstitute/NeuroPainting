...
CPU_SHARES = (
    DOCKER_CORES * 1024
)  # ECS computing units assigned to each docker container (1024 units = 1 core)
MEMORY = 15000  # Memory assigned to the docker container in MB
SECONDS_TO_START = (
    3 * 60
)  # Wait before the next CP process is initiated to avoid memory collisions
...
SQS_MESSAGE_VISIBILITY = (
    240 * 60
)  # Timeout (secs) for messages in flight (average time to be processed)
